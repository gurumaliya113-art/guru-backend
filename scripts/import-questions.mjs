...
function normalize(q, classLevel, subject) {
  ...
  const typeLabel = type === "fill" ? "Fill-in-the-Blank" : type === "truefalse" ? "True-False" : "MCQ";
  return {
    id: q.id || newId(),
    subject,
...
async function main() {
  ...
  for (const file of files) {
    ...
    const rows = list.map((q) => normalize(q, classLevel, subject)).filter((q) => q.text && q.options.some((o) => o));
    grand += rows.length;
